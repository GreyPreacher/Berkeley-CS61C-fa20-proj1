...
Color *evaluateOneCell(Image *image, int row, int col, uint32_t rule)
{
	...
	for (int i = 0; i < 8; i++){
		...
		if ((*(image->image + newRow * image->cols + newCol))->R == 255) aliveNeighbourR++;
		if ((*(image->image + newRow * image->cols + newCol))->G == 255) aliveNeighbourG++;
		if ((*(image->image + newRow * image->cols + newCol))->B == 255) aliveNeighbourB++;
	}
	idxR = isAliveR * 9 +aliveNeighbourR;
	idxG = isAliveG * 9 +aliveNeighbourG;
	idxB = isAliveB * 9 +aliveNeighbourB;
...
Image *life(Image *image, uint32_t rule)
{
	...
	newimg->image = (Color **)malloc(sizeof(Color*) * (newimg->cols) * (newimg->rows));
